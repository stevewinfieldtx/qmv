    ...
    def generate_music(self, preferences: Dict[str, Any], session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            suno_request = {
                ...
                'make_instrumental': music_prefs.get('vocal_style', 'none') == 'none',
                'wait_audio': True
            }
            ...
            if response.status_code == 200:
                ...
                # Suno returns 2 song variations by default
                ...
                for i, song in enumerate(songs[:2]):  # Ensure we only take 2
                    song_data = {
                        ...
                        'duration': song.get('duration'),  # Actual song duration from Suno
...
def process_music_generation(self, session_id: str):
    ...
    try:
        ...
        redis_client.setex(results_key, 86400, json.dumps(final_results))  # Store for 24 hours
        ...
        if self.request.retries < self.max_retries:
            logger.info(f"Retrying task for session {session_id} (attempt {self.request.retries + 1})")
